...
const deleteExamTypes = async () => {
    try {
        console.log("Deleting exam types...");
        await ExamType.deleteMany();

        console.log("Exam types deleted successfuly!");
    } catch (error) {
        console.error("An error occured while deleting exam types: ", error);
        process.exit(1);
    }
}
